import React from "react";
import { assets, dummyTestimonial } from "../../assets/assets";

function TestimonialsSection() {
  return (
    <div className="pb-14 px-8 md:px-0 max-w-6xl mx-auto">
      <h2 className="text-3xl font-medium text-gray-800 text-center">
        Testimonials
      </h2>
      <p className="md:text-base text-gray-500 mt-3 text-center">
        What Our Students Are Saying (Before They Get Too Famous): Don't just
        take our word for it.
        <br /> Read what these brave souls, who dared to transform their ideas
        with Presync Academy
      </p>
      <div className="mt-14">
        {/* Testimonials displayed side by side, 3 per row */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 justify-center">
          {dummyTestimonial.slice(0, 3).map((testimonial, index) => (
            <div
              className="text-sm text-left border border-gray-500/30 pb-6 rounded-lg bg-white shadow-[0px_4px_15px_0px] shadow-black/5 overflow-hidden h-full"
              key={index}
            >
              <div className="flex item-center gap-4 px-5 py-4 bg-gray-500/10">
                <img
                  className="h-12 w-12 rounded-full"
                  src={testimonial.image}
                  alt={testimonial.name}
                />
                <div>
                  <h1 className="text-gray-800 text-lg font-medium">
                    {testimonial.name}
                  </h1>
                  <p className="text-gray-800/80">{testimonial.role}</p>
                </div>
              </div>
              <div className="p-5 pb-7">
                <div className="flex gap-0.5">
                  {[...Array(5)].map((_, i) => (
                    <img
                      className="h-5"
                      key={i}
                      src={
                        i < Math.floor(testimonial.rating)
                          ? assets.star
                          : assets.star_blank
                      }
                      alt="star"
                    />
                  ))}
                </div>
                <p className="text-gray-500 mt-5">{testimonial.feedback}</p>
                <div className="mt-4 pl-0">
                  <a href="#" className="text-blue-500 underline">Read more</a>
                </div>
              </div>
            </div>
          ))}
        </div>
      </div>
    </div>
  );
}

export default TestimonialsSection;
